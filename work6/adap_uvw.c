...
void	ft_comp_uvw(void *param, int j, int i, int k)
{
	...
	if (ft_is_water(flags[j][i + 1][k]))
		speed_u[j][i][k] -= (press_p[j][i + 1][k] - press_p[j][i][k]) * deltat / dx;
	if (ft_is_water(flags[j + 1][i][k]))
		speed_v[j][i][k] -= (press_p[j + 1][i][k] - press_p[j][i][k]) * deltat / dy;
	if (ft_is_water(flags[j][i][k + 1]))
		speed_w[j][i][k] -= (press_p[j][i][k + 1] - press_p[j][i][k]) * deltat / dz;
