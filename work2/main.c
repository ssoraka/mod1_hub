...
void	ft_create_water_in_cell(void *ptr, int j, int i)
{
	...
	if (vis->inf->map[j][i] == WATER || vis->inf->map[j][i] == SURF)
	{
...
void ft_print_points(t_vis *vis, t_vektr *points)
{
	while (points)
	{
		ft_put_pixel_to_img(vis->addr, points->abs_x, points->abs_y, 0xFFFF);
		points = points->next;
	}
}
*/
...
void	ft_move_parts_x(t_fluid *fluid, t_vektr *p1, t_vektr *p2)
{
	...
	p2->x = p1->x + (fluid->deltat * (u * KOEFF));
	p2->otn_x = (int)p2->x;
	//printf("%lf + %d\n",p1->otn_x, i, j);
}
...
void	ft_move_parts_y(t_fluid *fluid, t_vektr *p1, t_vektr *p2)
{
	...
	p2->y = p1->y + (fluid->deltat * (v * KOEFF));
	p2->otn_y = (int)p2->y;
}


void	ft_move_parts(t_fluid *fluid, t_line *lines)
{
	t_vektr *tmp;
	int i;
	int j;

	i = lines->p1->x / DELTA + 1;
	j = lines->p1->y / DELTA;
	if (i >= 1 && j >= 1 && i <= fluid->imax && j <= fluid->jmax)
	{
		(fluid->parts[j][i])--;
		i = lines->p2->x / DELTA + 1;
		j = lines->p2->y / DELTA;
		(fluid->parts[j][i])++;
		tmp = lines->p2;
		lines->p2 = lines->p1;
