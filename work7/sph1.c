...
{
	part_i->delta_density += part_j->mass // part_j->density ///я ваще хз, откуда тут это взялось...
...
REAL	ft_calc_fake_viscosity(t_part *p_i, t_part *p_j, t_dpoint *d_speed, t_dpoint *d_pos)
{
	...
	h_ij = (p_i->h + p_j->h) / 2.0; // полусумма длин сглаживания частиц
	c_ij = (p_i->c + p_j->c) / 2.0; // полусумма скоростей звука частиц
	c_ij = (p_i->c + p_j->c) / 2.0; // полусумма скоростей звука частиц
	mu_ij = h_ij
	* ((d_speed->x * d_pos->x) / (d_pos->x * d_pos->z + CONST_E * CONST_E * h_ij * h_ij)
	+ (d_speed->y * d_pos->y) / (d_pos->y * d_pos->y + CONST_E * CONST_E * h_ij * h_ij)
	+ (d_speed->z * d_pos->z) / (d_pos->z * d_pos->z + CONST_E * CONST_E * h_ij * h_ij));
	viskosity = mu_ij * (CONST_B * mu_ij - CONST_A * c_ij) / press_ij;
	return (viskosity);
}


REAL	ft_calc_fake_viscosity2(t_part *p_i, t_part *p_j, t_dpoint *d_speed, t_dpoint *d_pos)
{
	REAL viskosity;
	REAL mu_ij; //коэффициент динамической вязкости
	REAL press_ij;
	...
	REAL tmp = ft_norma_vektora(d_pos->x, d_pos->y, d_pos->z);
	mu_ij = h_ij
	* (d_speed->x * d_pos->x + d_speed->y * d_pos->y + d_speed->z * d_pos->z)
	/ (tmp * tmp + CONST_E * CONST_E * h_ij * h_ij);
	printf("mu__%lf__\n", mu_ij);
	viskosity = mu_ij * (CONST_B * mu_ij - CONST_A * c_ij) / press_ij;
	return (viskosity);
}
...
REAL	ft_return_fake_viscosity(t_part *p_i, t_part *p_j)
{
	...
	dpos_x_dspeed = d_speed.x * d_pos.x + d_speed.y * d_pos.y + d_speed.z * d_pos.z;
	if (dpos_x_dspeed < 0.0)
	{
		viskosity = ft_calc_fake_viscosity2(p_i, p_j, &d_speed, &d_pos);
		printf("viskosity__%lf__\n", viskosity);
	}
	return (viskosity);
}
